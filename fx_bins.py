...
def numeric_var_binning(data, var, label, max_interval=10,method='Equifrequency',BRM=False,special_attribute=[],
    ...
    method: "ChiMerge"(卡方分箱);"DecisionTree"（决策树分箱);"Equifrequency"(等频分箱);"Equidistance"(等距分箱);"Mix"(混合分箱)
    ...
    bins: 分箱节点 list
    '''
    allowed_method = ["ChiMerge", "DecisionTree", "Equifrequency","Equidistance","Mix"]
...
def category_var_binning(data, var, label, max_interval=10, method='default',special_attribute=[],\
    ...
    ##给出每个箱中的包含哪些类别
    bins_df['contains_category'] = pd.Series(bins_df.index).replace(reverse_bins).values
    bins_df = bins_df[['contains_category','num_1', 'num_01']]
